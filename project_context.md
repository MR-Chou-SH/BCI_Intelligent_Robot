...
M6.0–M6.6b 已完成并保留 warnings。M6.6b 的真实 ND8 三 trial diagnostic 在 frozen NumPy FBCCA live chain 中产生 LEFT/CENTER/RIGHT 各一次正确 live decision；它是 engineering smoke evidence，不是 formal online accuracy。该晚一条电极物理松脱，session 实际只有四条有效 EEG 通道；这只说明 pipeline 在不理想硬件条件下仍工作，不能宣称四通道配置已验证。M6.7a 已实现 formal infrastructure，但尚未开始真实正式实验：60 s pre-session quality gate 从 candidate CH2/3/4/5/7 预先选择至少三条 usable channels，随后 channel set 对整个 session frozen；若其中任一条中途不可恢复失效则 abort。该 3/5 admission 是预声明工程规则，不是三通道充分性、已验证配置或与五通道等价的结论。M5/M6 的证据边界必须继续保留：`hardwareTimingVerified=false`、`physicalOpticalTimingVerified=false`，ND8 hardware sample anchor 与 hardware-exact timing 未验证，sample index 仅为 `software-derived estimate`，且名义刺激频率未获独立 optical measurement。当前 workspace 使用 NumPy 2.2.6 与 SciPy 1.14.1，但仓库尚无正式 requirements/pyproject dependency declaration，属于可复现性 warning。
