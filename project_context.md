...
M6.0 已完成 legacy / architecture audit。M6.1a 已完成真实设备验证并以 `Completed / PASS WITH WARNINGS` 收尾。M6.1b 已完成第一份受控三类别 EEG dataset：30 trials、每类 10 个，ground truth → Quest event → ND8 software-derived association 链路完整，并通过独立 dataset completeness verifier。M6.2a/2b 已在同一固定 30 个 epoch 上完成 Standard CCA 和 legacy-informed FBCCA baseline；两者在该单 session 中均为 30/30。M6.2 等待 closeout review，M6.3 尚未开始。该结果仅是固定配置下的 single-session separability evidence，不代表泛化、在线或硬件精确分类性能。M5/M6 的证据边界必须继续保留：`hardwareTimingVerified=false`、`physicalOpticalTimingVerified=false`，sample index 仅为 `software-derived estimate`。
