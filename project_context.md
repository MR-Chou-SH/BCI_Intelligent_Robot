...
M6.0–M6.6b 已完成并保留 warnings。M6.6b 的真实 ND8 三 trial diagnostic 在 frozen NumPy FBCCA live chain 中产生 LEFT/CENTER/RIGHT 各一次正确 live decision；它是 engineering smoke evidence，不是 formal online accuracy。该晚一条电极物理松脱，session 实际只有四条有效 EEG 通道；这只说明 pipeline 在不理想硬件条件下仍工作，不能宣称四通道配置已验证。M6 当前进入 M6.7 same-subject formal online validation 的协议冻结阶段，尚未开始真实正式实验。正式 protocol 要求所有 frozen CH2/3/4/5/7 在 60 s pre-session quality gate 通过；four-channel condition 仅允许 diagnostic/pilot 记录，不可进入 formal 30 trial。M5/M6 的证据边界必须继续保留：`hardwareTimingVerified=false`、`physicalOpticalTimingVerified=false`，ND8 hardware sample anchor 与 hardware-exact timing 未验证，sample index 仅为 `software-derived estimate`，且名义刺激频率未获独立 optical measurement。当前 workspace 使用 NumPy 2.2.6 与 SciPy 1.14.1，但仓库尚无正式 requirements/pyproject dependency declaration，属于可复现性 warning。
