...
M6.0 已完成 legacy / architecture audit。M6.1a 已完成真实设备验证并以 `Completed / PASS WITH WARNINGS` 收尾。M6.1b 已完成第一份受控三类别 EEG dataset：其 30/30 表示 dataset acquisition completeness，且 `classificationPerformed=false`。M6.2a/2b 已在同一固定 30 个 epoch 上完成 Standard CCA 和 legacy-informed FBCCA baseline；两者在该单 session 中均为 30/30。M6.3a 已完成固定 0.5–3.0 秒 window grid characterization：该 session 中 CCA 在 1.5 秒、FBCCA 在 1.0 秒达到 100%。M6.3b 已完成当前 NumPy 与稳定 legacy-style Chebyshev-I SOS zero-phase FBCCA realization 对比，短窗 0.5/1.0 秒出现 prediction disagreement，1.5/3.0 秒完全一致。M6.3 仍在进行中。该结果仅是当前 session 的 latency/filter-realization 与 strong within-session separability evidence，不代表最终系统、泛化、在线或硬件精确分类性能。M5/M6 的证据边界必须继续保留：`hardwareTimingVerified=false`、`physicalOpticalTimingVerified=false`，sample index 仅为 `software-derived estimate`。
