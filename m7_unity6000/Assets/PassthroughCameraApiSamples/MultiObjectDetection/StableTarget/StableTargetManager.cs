...
namespace BCIIntelligentRobot.Vision
{
    ...
    public sealed class StableTargetManager
    {
        ...
        /// <summary>
        /// Returns active, temporarily-missing, and lost records for lifecycle consumers.
        /// The existing GetCurrentTargets() semantics remain unchanged for active consumers.
        /// </summary>
        public IReadOnlyList<StableTargetSnapshot> GetAllTargets()
        {
            var snapshots = new List<StableTargetSnapshot>(m_Targets.Count);
            for (int i = 0; i < m_Targets.Count; i++)
            {
                snapshots.Add(CreateSnapshot(m_Targets[i]));
            }
