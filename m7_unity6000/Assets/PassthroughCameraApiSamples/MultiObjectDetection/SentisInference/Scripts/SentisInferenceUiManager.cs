...
namespace PassthroughCameraSamples.MultiObjectDetection
{
    ...
    public class SentisInferenceUiManager : MonoBehaviour
    {
        ...
        internal class RecentDetectionData
        {
            public string ClassName;
            public int ClassId;
            public Vector2 BboxCenterModelPixels;
        ...
        public void DrawUIBoxes(List<(int classId, Vector4 boundingBox)> detections, Vector2 inputSize, Pose cameraPose)
        {
            ...
            for (var i = 0; i < detections.Count; i++)
            {
                ...
                {
                    LogLocalization(classname, rect.center, normalizedCenter, ray, null);
                    continue;
                }
                LogLocalization(classname, rect.center, normalizedCenter, ray, worldPos);
                CacheRecentValidDetection(detection.classId, classname, rect.center, normalizedCenter, cameraPose);
        ...
        internal bool TryRaycastRecentDetection(out RecentDetectionData detection, out Ray ray, out Vector3 worldHitPoint, out float ageSeconds)
        {
            detection = m_recentValidDetection;
            ray = default;
            ...
            ageSeconds = Time.unscaledTime - detection.ValidAtTime;
            if (ageSeconds > RecentDetectionHoldSeconds)
            {
            ...
            // Re-run the official viewport-to-world path at A-button time; do not reuse an old world point.
            ray = m_cameraAccess.ViewportPointToRay(detection.ViewportPoint, detection.CameraPose);
            var worldPos = m_environmentRaycast.Raycast(ray);
            LogLocalization(detection.ClassName, detection.BboxCenterModelPixels, detection.ViewportPoint, ray, worldPos);
            if (!worldPos.HasValue)
            {
                return false;
            }

            worldHitPoint = worldPos.Value;
            return true;
        }
        ...
        private void CacheRecentValidDetection(int classId, string className, Vector2 bboxCenterModelPixels, Vector2 normalizedCenter, Pose cameraPose)
        {
            m_recentValidDetection = new RecentDetectionData
            {
                ClassId = classId,
                ClassName = className,
                BboxCenterModelPixels = bboxCenterModelPixels,
                ViewportPoint = new Vector2(normalizedCenter.x, 1f - normalizedCenter.y),
                CameraPose = cameraPose,
                ValidAtTime = Time.unscaledTime
            };
        }
        ...
        {
            string key = className + ":" + (hitPoint.HasValue ? "hit" : "miss");
            if (m_lastLocalizationLogTime.TryGetValue(key, out float lastLogTime) &&
                Time.unscaledTime - lastLogTime < LocalizationLogIntervalSeconds)
            {
                return;
            }

            m_lastLocalizationLogTime[key] = Time.unscaledTime;
            string common =
                "M7_OFFICIAL_LOCALIZATION class=" + className +
                " bbox_center_model_px=" + bboxCenterModelPixels.ToString("F1") +
            ...
            {
                Debug.Log(common + " raycast=hit world_hit_point=" + hitPoint.Value.ToString("F4"));
            }
            else
            {
