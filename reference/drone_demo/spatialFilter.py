...
#                     n_samples = epoch_band.shape[-1]
#                     template = np.squeeze(evoked[fbINX, :, int(self.srate*end):int(self.srate*end)+n_samples])
...
class fbCCA():
    def __init__(self, n_components=1, n_band=3, srate=250, frequency=[], lag=35, winLEN=2):
        ...
        self.montage = np.linspace(
            0, self.conditionNUM-1, self.conditionNUM).astype('int64')
