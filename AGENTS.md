...
Develop M7+ Unity work only in `m7_unity6000/`, the Unity 6000.0.66f2 official Passthrough Camera API sample baseline. M8.2b real Quest + ND8 engineering validation is completed/PASS WITH WARNINGS and the ViewLockedHud baseline is frozen; M8.3 is Completed/PASS. M8.4 now adds the Quest-only multi-target group and confirmed-batch UX above the immutable M8.3 result: Quest remains the authority for frozen class/index → TargetId facts, downstream consumers must not re-query live binding, and future robot work must consume only a user-confirmed batch. Do not repeat the M7.5 Passthrough/Camera API/YOLO/raycast baseline, revive M7.4 RGB→Environment Depth UV, start unscoped real ND8/Quest EEG experiments, or start robot control.
