...
M8.2b currently includes two experimental Quest visual-layout variants awaiting Quest-only acceptance: the existing world-space方案 A uses a `0.32 m` Quad, deterministic camera-right separation, and static cyan leader lines plus small anchor markers; the new `ViewLockedHud` mode uses three `0.20 m` Quads at camera-local `(-0.24, 0.18, 0.85)`, `(0, 0.18, 0.85)`, and `(0.24, 0.18, 0.85)` meters. In HUD mode the Quads follow the active camera while leader lines remain world-space and are recomputed each frame. The frame-driven `5/4/3` timing, black/white material, decoder, slot/class mapping and Quest snapshot contract are unchanged. Selection freeze holds the existing presentation-to-target association, while HUD transforms continue following the headset; terminal decision or explicit abort releases the association freeze. This is a visibility/layout diagnostic only; it does not claim an EEG-accuracy improvement or mark M8.2b complete.
