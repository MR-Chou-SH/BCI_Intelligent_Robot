...
The new `ViewLockedHud` presentation mode keeps the same binding and frame-driven renderer but parents only the three SSVEP Quads under an active-camera child. Its default camera-local positions are `(-0.24, 0.18, 0.85)`, `(0, 0.18, 0.85)`, and `(0.24, 0.18, 0.85)` meters with uniform `0.20 m` scale. Leader lines remain world-space and update their HUD endpoint each frame; target markers remain world-space and carry non-flashing slot numbers `1/2/3`. During selection freeze, the binding/line association is held from the existing lifecycle while the HUD root continues following the HMD. This implementation is ready for a separate Quest-only visual acceptance and does not claim an EEG-accuracy improvement.
