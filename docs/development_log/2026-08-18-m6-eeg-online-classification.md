...
## M6.1b closeout — controlled three-class dataset acquisition

The first formal acquisition, `m6_1b-dataset-20260819T080232Z-2f3cfd40`, is preserved as an `INCOMPLETE` audit record. ND8 acquisition, association readiness, Quest-PC synchronization and raw EEG were present, but Quest rejected the 1-based PC plan because its controller compared `trialIndex` against a 0-based loop index. Consequently it produced 0/30 Quest trials, no PC stimulus-event log, no derived associations and an incomplete verifier result. This was a cross-module semantic mismatch between the PC ground-truth protocol and Quest runtime validation, not merely an application-launch failure.

The validated runtime fix corrected 1-based external trial-index validation, adopted the PC ground-truth session ID for formal Quest events, reset the formal event sequence to zero, added runtime diagnostics and dirty-worktree manifest evidence, and required Quest-PC affine synchronization before the formal preparation window. The no-ND8 diagnostic evidence under `D:\EEG_Study\m6_1b_diagnostic\...` passed: a 30-trial 10/10/10 plan was sent and accepted; the 1-trial LEFT/7.2 Hz regression and the 3-trial LEFT → CENTER → RIGHT regression produced complete valid, in-order event pairs with matching session IDs and no Unity runtime exceptions or crash.

The successful formal session was `m6_1b-dataset-20260819T090215Z-d70eaa0d` with seed `20260819`, collected on COM11 using Neurodance 7.3 / CPython 3.9.13 at 1000 Hz. It retained 2,087 packets of 8 × 200 samples (417,400 samples/channel), continuous ND8 packet evidence in post-sync segment 2, 30/30 trials, 10/10/10 class balance, 30 start events, 30 stop events and 60/60 valid software-derived associations. Quest-PC synchronization recorded 361 accepted samples with final affine residual RMS approximately 12.9 ms. The independent verifier wrote `dataset-completeness.json` with `status=complete`, `errors=[]` and `warnings=[]`; no classification was performed.

The formal raw session was collected from a validated dirty working tree and its manifest records the changed/untracked files and tracked diff SHA-256. Basic channel sanity for this session reported usable/nonconstant CH2/3/4/5/7 and placeholder candidates CH0/6, with an overall `usable_with_warnings` recommendation. The current session differed slightly from prior M6.1a channel behavior; no channel optimization or causal interpretation was performed.

M6.1b is therefore `Completed / PASS WITH WARNINGS`. The warnings concern dirty-working-tree reproducibility, conservative channel sanity, and the continuing hardware/optical timing boundary. Hardware timing, physical optical timing, ND8 hardware sample anchoring, EEG class separability and classification performance remain unverified. Engineering lessons include making plan acceptance a mandatory preflight, treating cross-module indexing as scientific metadata, separating connectivity from controller readiness, and independently verifying evidence completeness. No new ADR was required because the existing protocol and ADR-003 sufficiently define PC ground truth authority and timing evidence boundaries.

## M6.6b real-device diagnostic closeout (2026-08-20)

Session `m6_6b-live-20260820T143946Z-ac87baf2` is `Completed / PASS WITH WARNINGS`. It was a fixed-order three-trial engineering smoke test, not a randomized formal accuracy experiment. The real chain Quest stimulus → Quest-PC synchronization → software-derived association → live ND8 packets → rolling NumPy FBCCA → 0.2 s predictions → 2-Consecutive produced one immutable decision for each target. LEFT, CENTER and RIGHT each had 11 prediction opportunities; their first two labels agreed with the plan target and final decisions were each at the software logical 2.2 s point. Post-hoc result was 3/3 correct, with 6/6 valid associations and no runtime/callback error.

The session must be interpreted with its actual hardware condition: the user reports that one intended electrode physically detached during wearing, leaving four effective EEG channels. The observed near-rail SDK channel is consistent with an unusable/disconnected-channel candidate, but no reliable physical-electrode-to-SDK-channel map exists and none is inferred. The frozen decoder was not retuned, and this result does not validate a four-channel BCI configuration; it is only an engineering robustness observation under degraded hardware.

The 2.2 s result is `0.5 s guard + 1.5 s analysis + 0.2 s second prediction step`, i.e. algorithmic logical time relative to software stimulus start. Steady-state second predictions took approximately 4.5–6.8 ms compute time; the first LEFT call took about 146.9 ms and is treated as an initialization/warm-up candidate, not physical latency. Physical optical onset, ND8 acquisition/USB latency, hardware sample-anchor offset, and true end-to-end latency remain unmeasured.

M6.7 formal online validation is protocol-defined in `docs/protocols/M6_7_formal_online_validation_protocol.json` and not started. Total control skipped the separate 9-trial pilot because M6.6b already supplied the small-scale real-device live-chain diagnostic; the next real experiment is an independent randomized 30-trial 10/10/10 same-subject prospective session. After the first three completed formal trials, a predeclared engineering-only checkpoint checks transport, sync freshness, valid associations, continuity, frozen-channel health, evidence, duplicate decisions, and runtime errors. It cannot inspect correctness, accuracy, or classification outcome: if that technical chain is healthy, all remaining 27 trials continue even after wrong or absent decisions. Technical invalids have a predeclared maximum of three replacements; decoder errors and no-decisions are never replaced. Hardware/optical timing is not a blocker for this explicitly software-timed online validation, but remains required before paper-level physical latency claims.

## M6.7 stress-online closeout (2026-08-21)

The independently preserved `stress_online` session `m6_7-formal-20260820T160940Z-0ef360f6` completed 30 randomized trials with LEFT/CENTER/RIGHT = 10/10/10. The user was in a tired/non-ideal attention state, so this is retained as non-ideal-condition engineering stress evidence, not primary formal online accuracy. The 60 s admission independently selected and froze CH2/CH4/CH7: CH3 and CH5 were constant/near-rail. All 30 trials were technically valid, all 30 had decisions, there were no replacements or no-decisions, and post-hoc evaluation was 30/30 correct. The early technical checkpoint after trials 1--3 returned CONTINUE without inspecting classification correctness. Logical decisions occurred at 2.2 s; this is software-side logical latency, not physical end-to-end latency.

Two earlier M6.7 incomplete preflight sessions caused by real ND8 disconnects remain unchanged as incomplete evidence. A separate ND8-only 120 s stability check then passed with 594 packets, 593 continuous packets, one startup anomaly, and no callback/runtime error. The reusable `eeg/acquisition/nd8_stability_check.py` utility is included; it performs no Quest control, plan broadcast, or decoding and requires the requested duration before reporting PASS.

M6.7 closes `Completed / PASS WITH WARNINGS`. The frozen EEG decoder chain is sufficiently exercised to support the next separately scoped BCI decision → robot command interface integration. This does not validate three-channel equivalence, cross-subject/generalized performance, hardware/optical timing, or physical end-to-end latency. No M7 implementation is started by this closeout.
