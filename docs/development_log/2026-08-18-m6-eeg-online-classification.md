...
M6.7 formal online validation is protocol-defined in `docs/protocols/M6_7_formal_online_validation_protocol.json` and not started. It requires a separate no-tuning 9-trial pilot, followed only after a fresh all-five-channel quality admission by a randomized 30-trial 10/10/10 same-subject prospective session. Technical invalids have a predeclared maximum of three replacements; decoder errors and no-decisions are never replaced. Hardware/optical timing is not a blocker for this explicitly software-timed online validation, but remains required before paper-level physical latency claims.
